...
func StartEsSync(db *gorm.DB, cfg *config.Config, log *zap.Logger) {
	if !cfg.ES.Enabled {
		return
	}
	go func() {
...
func fullSync(db *gorm.DB, cfg *config.Config, log *zap.Logger) {
...
func incSync(db *gorm.DB, cfg *config.Config, log *zap.Logger) {
...
func indexPost(cfg *config.Config, p entity.Post) error {
