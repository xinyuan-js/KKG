...
func (h *Handler) loginUserOrNil(c *gin.Context) *entity.User {
	token := accessTokenFromGinContext(c)
	if token == "" {
		return nil
	}
	data, err := h.sessions.ValidateAccess(c.Request.Context(), token)
	if err != nil || data.UserID == 0 {
		return nil
	}
	u, err := h.userSvc.EnsureFromSharedUserID(int64(data.UserID))
	if err != nil {
		return nil
	}
	return u
...
func accessTokenFromGinContext(c *gin.Context) string {
