...
func (h *Handler) UserLogin(c *gin.Context) {
	...
	mustNoErr(err)
	sharedID, err := h.userSvc.EnsureSharedUserForOJUser(u, req.UserPassword)
	mustNoErr(err)
	mustNoErr(h.setAuthCookies(c, sharedID, u.UserRole))
...
func (h *Handler) UserWxLogin(c *gin.Context) {
	...
	sharedID, err := h.userSvc.EnsureSharedUserForOJUser(&u, "wx-"+unionID)
	mustNoErr(err)
	mustNoErr(h.setAuthCookies(c, sharedID, u.UserRole))
	c.JSON(http.StatusOK, common.Success(loginUserVO(&u)))
}
