...
func RegisterRoutes(r *gin.Engine, app *bootstrap.App, deps dependencies) {
	...
	v1 := r.Group("/api/v1")
	{
		...
		registerOJRoutes(v1, app, deps.ojHandler)
	}
}
...
func registerAuthRoutes(v1 *gin.RouterGroup, app *bootstrap.App, deps dependencies) {
	authGroup := v1.Group("/auth")
	{
		...
		authGroup.GET("/me", middleware.Session(app.Sessions), middleware.RequireActiveUser(app.DB), deps.authHandler.GetProfile)
	}
}
...
func registerPublicRoutes(v1 *gin.RouterGroup, app *bootstrap.App, deps dependencies) {
	...
	v1.GET("/feed", middleware.OptionalSession(app.Sessions), deps.postHandler.ListFeed)
	v1.GET("/rankings/posts", deps.postHandler.Ranking)
	v1.GET("/posts/:id", deps.postHandler.Get)
	v1.GET("/posts/:id/engagement", middleware.OptionalSession(app.Sessions), deps.postHandler.GetEngagement)
	...
	v1.GET("/search", middleware.OptionalSession(app.Sessions), deps.searchHandler.Search)
	v1.GET("/search/suggest", middleware.OptionalSession(app.Sessions), deps.searchHandler.Suggest)
}

func registerAuthedRoutes(v1 *gin.RouterGroup, app *bootstrap.App, deps dependencies) {
	authed := v1.Group("")
	authed.Use(middleware.Session(app.Sessions))
